...
const groq = new Groq({
  apiKey: "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", // Store securely in .env
});
...
router.post("/content/:userId", userAuth, async (req, res) => {
  ...
  try {
    ...
    if (greetings.includes(normalizedPrompt)) {
      ...
      await Chat.create({ userId, agentId: AGENT_ID, prompt, response: greetingResponse, tokensUsed: 0 });
    ...
    const isFollowUp = followUpKeywords.some((keyword) => normalizedPrompt.includes(keyword));
    ...
    if (isFollowUp) {
      if (promptHistory.has(userId)) {
        ...
        await Chat.create({ userId, agentId: AGENT_ID, prompt, response: noHistoryMsg, tokensUsed: 0 });
        return res.status(400).json({ response: noHistoryMsg, tokensLeft: user.tokens });
      }
    }
    ...
    const classification = await groq.chat.completions.create({
      messages: [
        {
          role: "system",
          content: "You are an intent classifier. If the user is asking for a caption reply with 'yes'. Otherwise reply with 'no'.",
        },
    ...
    const intent = classification.choices[0]?.message?.content?.trim().toLowerCase() || "no";
    ...
    if (intent !== "yes") {
      ...
      if (lastChats.length > 0) {
        const formattedChats = lastChats
          .map((chat) => `📝 Image desc: ${chat.prompt}\n💡 Caption: ${chat.response}`)
          .join("\n\n---\n\n");
        const fallbackMessage = `⚠️ I only generate image captions.\n\nHere are your last 3 captions:\n\n${formattedChats}`;
        await Chat.create({ userId, agentId: AGENT_ID, prompt, response: fallbackMessage, tokensUsed: 0 });
        return res.json({ response: fallbackMessage, tokensLeft: user.tokens });
      } else {
        const noHistoryMessage = "⚠️ I only generate image captions. You haven't created any yet.";
        await Chat.create({ userId, agentId: AGENT_ID, prompt, response: noHistoryMessage, tokensUsed: 0 });
        return res.status(400).json({ response: noHistoryMessage, tokensLeft: user.tokens });
      }
    }
    ...
    try {
      ...
      if (!freshUser) {
        ...
        session.endSession();
        return res.status(404).json({ error: "User not found during transaction." });
      }
      if (freshUser.tokens < 10) {
        await session.abortTransaction();
        session.endSession();
        return res.status(403).json({ error: "Insufficient tokens during processing." });
      }
      ...
      return res.status(500).json({ error: "Error saving chat or deducting tokens." });
    }
    ...
    return res.status(200).json({ response: fullResponse, tokensLeft: user.tokens });
  } catch (err) {
