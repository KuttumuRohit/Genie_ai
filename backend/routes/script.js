...
const groq = new Groq({
    apiKey:"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", // Keep it secure in .env
});
...
router.post("/content/:userId", userAuth, async (req, res) => {
  ...
  try {
    ...
    // Follow-up logic
    const followUps = [
      "expand it", "make it bigger", "increase size", "add more content",
      "add more examples", "make it long", "continue it", "add details", "elaborate",
    ];
    ...
    let intent = "no";
    const scriptKeywords = ["script", "podcast", "youtube", "short", "reel", "dialogue", "conversation"];
    const containsScriptKeyword = scriptKeywords.some((kw) => normalizedPrompt.includes(kw));
    ...
    try {
      ...
      console.log(`🧠 Intent classifier said: "${intent}" for prompt: ${finalPrompt}`);
    } catch (clsErr) {
    ...
    if (intent !== "yes" && !containsScriptKeyword) {
      ...
      const noHistoryMessage = "⚠️ I only generate scripts... You haven't generated any yet.";
    ...
    const stream = await groq.chat.completions.create({
      messages: [
        { role: "system", content: "You are a professional script writer. Generate a complete, engaging, and creative script based on the user's prompt." },
        { role: "user", content: finalPrompt },
      ],
